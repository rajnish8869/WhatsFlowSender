...
export const ComposeView: React.FC<Props> = ({ state, dispatch }) => {
  ...
  // Local state for the message to avoid global re-renders on every keystroke
  const [localMessage, setLocalMessage] = useState(state.messageTemplate);

  // Local state for preview URL to manage lifecycle
  const [previewUrl, setPreviewUrl] = useState<string>("");

  // Refs to ensure cleanup effect has access to latest values
  ...
  }, [localMessage, dispatch]);

  // Sync to global state with debounce
  useEffect(() => {
    const handler = setTimeout(() => {
      dispatchRef.current({ type: "SET_MESSAGE", payload: localMessage });
    }, 500);
    return () => clearTimeout(handler);
  }, [localMessage]);

  // Sync to global state on unmount/blur to ensure no data loss on quick navigation
  useEffect(() => {
    return () => {
      dispatchRef.current({
        type: "SET_MESSAGE",
        payload: localMessageRef.current,
      });
    };
  }, []);

  // Manage attachment preview URL lifecycle to prevent memory leaks
  useEffect(() => {
    const file = state.attachment?.file;
    if (file) {
    ...
    if (file) {
      dispatch({
        ...
        payload: {
          ...
          type: file.type,
          file,
          previewUrl: "", // Handled by useEffect locally
        },
      });
  ...
  const insertText = (textToInsert: string, wrap: boolean = false) => {
    if (textareaRef.current) {
      ...
      const currentText = localMessage;
  ...
  const handleSaveTemplate = () => {
    if (localMessage.trim()) {
      dispatch({ type: "SAVE_TEMPLATE", payload: localMessage });
  ...
  const isValid =
    selectedCount > 0 &&
    (localMessage.trim().length > 0 || state.attachment !== null);
        ...
        <div className="absolute inset-0 z-40 bg-white dark:bg-zinc-950 animate-slide-up flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800">
            <h3 className="font-bold text-lg flex items-center gap-2">
              <BookTemplate size={20} className="text-emerald-500" /> Saved
              Templates
            </h3>
            <button
            ...
            {state.templates.length === 0 ? (
              <div className="text-center py-10 text-zinc-400">
                ...
                <p className="text-xs mt-2">
                  Write a message and click the Save icon.
                </p>
              </div>
            ) : (
              state.templates.map((tmpl, idx) => (
                <div
                  key={idx}
                  className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 flex flex-col gap-3 group"
                >
          ...
          <div className="flex-none border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 p-2 flex flex-wrap gap-2 items-center z-10">
            ...
            <div className="flex items-center bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-0.5 shadow-sm">
              <button
                ...
                className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 transition-colors"
                title="Bold"
              >
                <Bold size={16} />
              </button>
              <button
                onClick={() => insertText("_", true)}
                className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 transition-colors"
                title="Italic"
              >
                <Italic size={16} />
