       ...
       <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 pb-32">
          ...
          <div className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden flex flex-col relative focus-within:ring-2 focus-within:ring-emerald-500/50 transition-all shadow-sm min-h-[250px]">
             ...
             <div className="h-12 border-b border-zinc-100 dark:border-zinc-800 flex items-center px-2 gap-2 bg-zinc-50 dark:bg-zinc-900/50 flex-none">
                <button onClick={insertVariable} className="px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 transition-colors border border-zinc-200 dark:border-zinc-700 shadow-sm">
                  <Wand2 size={12} /> {'{name}'}
                </button>
                <div className="flex-1" />
                <input type="file" className="hidden" ref={fileInputRef} onChange={handleFile} />
                <button onClick={() => fileInputRef.current?.click()} className={`p-2 rounded-lg transition-colors ${state.attachment ? 'text-emerald-500 bg-emerald-500/10' : 'text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
                   <Paperclip size={18} />
                </button>
             </div>
             ...
             {state.attachment && (
               <div className="absolute bottom-4 left-4 right-4 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md rounded-lg p-3 flex items-center gap-3 border border-zinc-200 dark:border-zinc-700 animate-slide-up shadow-lg">
                  <div className="w-10 h-10 rounded bg-zinc-100 dark:bg-black/50 flex items-center justify-center overflow-hidden">
                    {state.attachment.type.startsWith('image') ? (
                      <img src={state.attachment.previewUrl} className="w-full h-full object-cover" />
                    ) : <ImageIcon size={20} className="text-zinc-500" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-zinc-900 dark:text-white truncate">{state.attachment.name}</p>
                    <p className="text-[10px] text-zinc-500 uppercase">{state.attachment.type.split('/')[1]}</p>
                  </div>
                  <button onClick={() => dispatch({ type: 'SET_ATTACHMENT', payload: null })} className="p-1 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full">
